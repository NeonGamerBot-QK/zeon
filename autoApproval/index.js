  ...
  app.on(['pull_request.opened', 'pull_request.reopened', 'pull_request.labeled', 'pull_request.edited', 'pull_request_review'], async (context) => {
    ...
    if (config.required_labels_mode === 'one_of') {
      ...
      const appliedRequiredLabels = config.required_labels
        .filter((requiredLabel) => prLabels.includes(requiredLabel))
...
function applyAutoMerge (context, prLabels, mergeLabels, rebaseLabels, squashLabels) {
  if (mergeLabels && prLabels.filter((label) => mergeLabels.includes(label)).length > 0) {
    enableAutoMerge(context, 'MERGE')
  }
  if (rebaseLabels && prLabels.filter((label) => rebaseLabels.includes(label)).length > 0) {
    enableAutoMerge(context, 'REBASE')
  }
  if (squashLabels && prLabels.filter((label) => squashLabels.includes(label)).length > 0) {
...
async function enableAutoMerge (context, method) {
...
async function getAutoapprovalReviews (context) {
  const pr = context.pullRequest()
  const reviews = await context.octokit.pulls.listReviews(pr)

  const autoapprovalReviews = (reviews.data).filter((item) => item.user.login === 'neonzeon[bot]')
