...
const deleteMergedBranch = async (context) => {
  const config =
    (await context.config("zeon/pr.yml", {
      exclude: [],
      on_base: [],
  ...
  if (headRepoId !== baseRepoId) {
    context.log.info(
      `Closing PR from fork. Keeping ${context.payload.pull_request.head.label}`,
    );
    return;
  }
  ...
  if (
    config.on_base &&
    config.on_base.length > 0 &&
    !config.on_base.some((rule) =>
      new RegExp(`^${rule.split("*").join(".*")}$`).test(baseBranchName),
    )
  ) {
    context.log.info(
      `Base does not match any 'on_base'. Keeping ${context.payload.pull_request.head.label}`,
    );
    return;
  }
  ...
  if (!isMerged && config.delete_closed_pr === false) {
    context.log.info(
      `PR was closed but not merged. Keeping ${owner}/${repo}/${ref}`,
    );
    return;
  }
  ...
  try {
    ...
    context.log.info(
      `Successfully deleted ${owner}/${repo}/${ref} which was ${isMerged ? "merged" : "closed"}`,
    );
  } catch (e) {
