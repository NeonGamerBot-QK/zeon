  ...
  app.on(["check_suite.completed"], async (ctx) => {
    ...
    for (const pr of prs) {
      ...
      if (
        ...
        app.log.info(`Skipping PR #${pr.number} — [do-not-automerge] tag present`);
        await ctx.octokit.issues.createComment({
          owner: repository.owner.login,
          repo: repository.name,
      ...
      try {
        ...
        app.log.error(`Failed to fetch diff for PR #${pr.number}: ${e.message}`);
        continue;
      }
      ...
      try {
        ...
        app.log.error(`Failed to post comment on PR #${pr.number}: ${e.message}`);
      }
      ...
      if (isAccepted) {
        try {
          ...
          app.log.error(`Failed to approve/merge PR #${pr.number}: ${e.message}`);
        }
      } else {
        try {
          await ctx.octokit.pulls.createReview({
          ...
          app.log.error(`Failed to request changes on PR #${pr.number}: ${e.message}`);
        }
      }
    }
  });
