...
I require pull request titles to follow the [Conventional Commits specification](https://www.conventionalcommits.org/en/v1.0.0/) and it looks like your proposed title needs to be adjusted as  without it being in that order i can read your PR correctly.
        ...
        try {
          context.octokit.repos.createOrUpdateFileContents(
            context.repo({
              ...
              content: Buffer.from(`# Autogenerated file. \n# Zeon created this file.\nko_fi: saahil\ngithub: [NeonGamerBot-QK]`).toString("base64"),
            }),
          );
        } catch (e) {
          console.error(e);
  ...
  app.on(["push"], async (ctx) => {
    ...
    if (
      push.commits.some((c) => c.message.includes("zeon:codeowners")) && ctx.payload.pusher.name
    ) {
...
let appendToCodeOwners = filesMatched.map(f => `${f.filename} @${ctx.payload.pusher.name}`).join('\n')
await ctx.octokit.repos.createOrUpdateFileContents(ctx.repo({
  path: ".github/CODEOWNERS",
  message: `ci(git): update codeowners`,
  sha: currentFileContent.data.sha,
  content: Buffer.from(currentFileContent.data.content+'\n'+appendToCodeOwners).toString('base64'),
}))
      // console.log(`codeowners`, compare.data.files);
    }
    if( push.commits.some((c) => c.message.includes("zeon:ai_comment"))) {

      const fc = push.commits.find((c) => c.message.includes("zeon:ai_comment"));
      //todo convert to octokti
      const messages =  [{ role: 'user', content: await ctx.octokit.request('GET /repos/{owner}/{repo}/commits/{ref}', ctx.repo({
        ref: fc.id,
        headers: {
          'X-GitHub-Api-Version': '2022-11-28',
          'Accept': 'application/vnd.github.patch'
        }
      })).then(e =>e.data) }, { role: 'user', content: `create a commit comment off patch .make it use new lines. You are zeon.` }]
      ...
      console.log(chatCompletion.choices[0].message.content, 'rip tokens used on this commit message')
            ctx.octokit.repos.createCommitComment({
        commit_sha: ctx.payload.after,
        repo: ctx.payload.repository.name,
